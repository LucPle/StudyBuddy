...
int main(int argc, char* argv[]) {
    ...
    sock = socket(PF_INET, SOCK_STREAM, 0);     //step 1. 소켓 설정
    if (sock == -1)
        error_handling("socket() error");

    memset(&serv_addr, 0, sizeof(serv_addr));   //step 2. 소켓을 주소에 binding
    ...
    while (1) {
        str_len = read(sock, server_msg, sizeof(server_msg));         //step 3. 소켓 값 받아오기
        
        if (str_len == -1)
            error_handling("read() error");
        printf("Receive from Server : %s\n", server_msg);
        if (strncmp(server_msg, "1",1) == 0) {       //서버 전달값이 1이면 센서코드 실행
            printf("start\n");
            ...
            if (disc_state == 0 && light_state == 1) {      //조도만 문제 있을때
                snprintf(client_msg, 2, "%d", 1);
            }
            else if (disc_state == 1 && light_state == 0) { //불쾌지수만 문제있을때
                snprintf(client_msg, 2, "%d", 2);
            }
            else if (disc_state == 1 && light_state == 1) { //둘다 문제있을때
                snprintf(client_msg, 2, "%d", 3);
            }
            else {  //아무 문제 없을때..
                snprintf(client_msg, 2, "%d", 0);
            }
            printf("Send message to Server : %s\n", client_msg);
            if (write(sock, client_msg, sizeof(client_msg)) == -1) {
...
(반복문)
3. 서버 값 받아오기(read)
4. 서버 값이 1이면 센서 코드 실행
    여기서 센서 값에 따라 소켓 전송(write)
5. 서버 값이 0이면 계속 wait
